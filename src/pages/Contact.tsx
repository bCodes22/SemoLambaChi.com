          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12">
            {officers.map((officer) => (
              <div key={officer.email} className="text-center group">
                ...
                <h3 className="text-xl font-medium text-gray-900">{officer.name}</h3>
                <p className="text-gray-600 mb-4">{officer.position}</p>
                <a
                  href={`mailto:${officer.email}`}
                  aria-label={`Email ${officer.name}`}
                  className="inline-flex items-center justify-center w-10 h-10 rounded-full text-gray-500 hover:text-white hover:bg-gray-900 transition-colors duration-300"
                >
